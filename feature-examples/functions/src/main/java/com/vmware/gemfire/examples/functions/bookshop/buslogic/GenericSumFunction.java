...
public class GenericSumFunction implements Function {
  ...
  public void execute(FunctionContext context) {
    if (context instanceof RegionFunctionContext) {
      ...
      Region<Object, BookOrder> localRegion = PartitionRegionHelper.getLocalDataForContext(rfc);
      BigDecimal summable = BigDecimal.ZERO;
      for (BookOrder item : localRegion.values()) {
        float totalPrice = item.getTotalPrice();
        // if (field instanceof Float) {
        summable = summable.add(BigDecimal.valueOf(totalPrice));
        // } else {
  ...
  /*
   * @Override public void init(Properties props) { // Nothing to do
   *
   * }
   */
}
